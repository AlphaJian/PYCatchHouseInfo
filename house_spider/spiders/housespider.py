...
class HouseSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in response.xpath("//ul[@class='js_fang_list']//div[@class='info']"):
            model['title'] = item.xpath(".//a[@class='text link-hover-green js_triggerGray js_fanglist_title']/text()").extract_first()
            ...
            model['totalprice'] = item.xpath(".//span[@class='total-price strong-num']/text()").extract_first() +\
                                  item.xpath(".//span[@class='unit']/text()").extract_first()
            model['unitprice'] = item.xpath(".//span[@class='info-col price-item minor']/text()").extract_first()
        ...
        next_page_url = response.xpath("//a[@gahref='results_next_page']/@href").extract_first().encode("utf-8")
        if next_page_url is not None:
            yield scrapy.Request(self.host_url + next_page_url)
